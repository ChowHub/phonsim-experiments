<!----
Template takes..
scored.dat: data frame with means over trialtype for each subject
conds:  conditions to use
dv:  vector specifying what dv to run the anova for
--->

```{r}
opts_knit$set(root.dir=".")
opts_chunk$set(fig.cap="", fig.width=5, fig.height=3.5, dev='svg')
```
...
```{r}
y_labels = c(ACC.ser = 'Serial Accuracy', ACC.item = 'Item Accuracy', ACC.ord = 'Order Accuracy')
trialtype_labels = c(S = 'Similar', D = 'Dissimilar')
scored.dat$trialtype = relevel(scored.dat$trialtype, ref='S')
scored.dat$task = revalue(scored.dat$task, pub_conds)
scale_args = list("Trial Type", breaks = c('S', 'D'), labels = c('Similar', 'Dissimilar'))

if ('R.pool350' %in% scored.dat$task){
  poolsize = c(10, 15, 30, 60, 350)
  scored.dat$task = factor(as.integer(gsub('R.pool', "", scored.dat$task)), poolsize)
...
for (dv in dvs){
  ...
  p.fx = p.summary %+% meanse + 
    ...
    do.call(scale_shape_manual, c(scale_args, list(values=c(17, 16)))) +
    labs(y=y_labels[dv], x='Task')
  print(p.fx)
  all_plots[[dv]] = p.fx
}
save(all_plots, file=paste0('summary/', scored.dat$folder[1], '_plot_', dv,'.Rdata'))
```
